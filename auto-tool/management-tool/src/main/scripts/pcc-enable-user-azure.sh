...
do
   ...
   a) AZURE_ACCESS_ID="$OPTARG" ;;
   s) AZURE_SECRET_KEY="$OPTARG" ;;
   h) HELP="TRUE"; break;;
   esac
done
shift  $(($OPTIND - 1))
...
if [ $OPT_ERROR ]; then
        ...
        echo "Example: `basename $0` -u username -P azure"
        echo "任意のACCESS_IDとSECRET_KEYを用いる場合。"
        echo "Example: `basename $0` -u username -P azure -a <ACCESS_ID> -s <SECRET_KEY>"
        exit 1
fi
...
if [ -n "${USER_NAME}" -a -n "${PLATFORM_NAME}" ]; then
        ...
        if [ -z "${AZURE_ACCESS_ID}" -a -z "${AZURE_SECRET_KEY}" ]; then
            AZURE_ACCESS_ID=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -config AZURE_ACCESS_ID"`
            if [ "${AZURE_ACCESS_ID}" = "NULL" ]; then
                echo "AZURE_ACCESS_IDの読み込みに失敗しました。"
                exit 1
            fi

            AZURE_SECRET_KEY=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -config AZURE_SECRET_KEY"`
            if [ "${AZURE_SECRET_KEY}" = "NULL" ]; then
                echo "AZURE_SECRET_KEYの読み込みに失敗しました。"
                exit 1
            fi
        else
            #ACCESS_IDかSECRET_KEYの指定が無ければエラー
            if [ -z "${AZURE_ACCESS_ID}" ]; then
                echo "ACCESS_IDの値を入力してください。"
                exit 1
            elif [ -z "${AZURE_SECRET_KEY}" ]; then
        ...
        #プラットフォーム番号の取得
        PLATFORM_NO=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -platformname \"${PLATFORM_NAME}\" -platformkind azure"`
        ...
        SQL_ALREADY_AZURE_CREATED="SELECT USER_NO FROM AZURE_CERTIFICATE where USER_NO =${USER_NO} and PLATFORM_NO=${PLATFORM_NO}"
        USER_NO_CHECK=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -S -sql \"${SQL_ALREADY_AZURE_CREATED}\" -columntype int -columnname USER_NO"`
        ...
        #AZUREユーザー認証情報の作成
        SQL_CREATE_AZURE_CERTIFICATE="INSERT INTO AZURE_CERTIFICATE values (${USER_NO}, '${PLATFORM_NO}','${AZURE_ACCESS_ID}','${AZURE_SECRET_KEY}',NULL,'${USER_NAME}','${SSH_KEY_PUB}')"
        MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -U -sql \"${SQL_CREATE_AZURE_CERTIFICATE}\""`
        ...
        #AZURE用のキーペアAPIがないのでコメント
        #AZURE用のキーペアをAPIを通して送る
#        MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -I -keyname \"${USER_NAME}\" -publickey \"${SSH_KEY_PUB}\" -userno \"${USER_NO}\" -platformno \"${PLATFORM_NO}\" -platformkind azure"`

#        if [ -n "${MESSAGE}" ]; then
#            SQL_DELETE_AZURE_CERTIFICATE="DELETE FROM AZURE_CERTIFICATE where USER_NO = ${USER_NO} and PLATFORM_NO = ${PLATFORM_NO}"
...
#                echo "${USER_NAME} キーのインポートに失敗しました。ACCESS_ID、SECRET_KEYの値を確認して下さい。"
                #エラーの場合はAZUREユーザー認証情報を削除する
#                MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -U -sql \"${SQL_DELETE_AZURE_CERTIFICATE}\""`
#                if [ -n "${MESSAGE}" ]; then
#                        echo "${MESSAGE}"
#                fi
#                exit 1
