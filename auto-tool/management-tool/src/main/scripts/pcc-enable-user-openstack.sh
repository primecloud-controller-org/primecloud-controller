...
do
   ...
   a) OPENSTACK_ACCESS_ID="$OPTARG" ;;
   s) OPENSTACK_SECRET_KEY="$OPTARG" ;;
   h) HELP="TRUE"; break;;
   esac
done
shift  $(($OPTIND - 1))
...
if [ $OPT_ERROR ]; then
        ...
        echo "Example: `basename $0` -u username -P openstack"
        echo "任意のACCESS_IDとSECRET_KEYを用いる場合。"
        echo "Example: `basename $0` -u username -P openstack -a <ACCESS_ID> -s <SECRET_KEY>"
        exit 1
fi
...
if [ -n "${USER_NAME}" -a -n "${PLATFORM_NAME}" ]; then
        ...
        if [ -z "${OPENSTACK_ACCESS_ID}" -a -z "${OPENSTACK_SECRET_KEY}" ]; then
            OPENSTACK_ACCESS_ID=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -config OPENSTACK_ACCESS_ID"`
            if [ "${OPENSTACK_ACCESS_ID}" = "NULL" ]; then
                echo "OPENSTACK_ACCESS_IDの読み込みに失敗しました。"
                exit 1
            fi

            OPENSTACK_SECRET_KEY=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -config OPENSTACK_SECRET_KEY"`
            if [ "${OPENSTACK_SECRET_KEY}" = "NULL" ]; then
                echo "OPENSTACK_SECRET_KEYの読み込みに失敗しました。"
                exit 1
            fi
        else
            #ACCESS_IDかSECRET_KEYの指定が無ければエラー
            if [ -z "${OPENSTACK_ACCESS_ID}" ]; then
                echo "ACCESS_IDの値を入力してください。"
                exit 1
            elif [ -z "${OPENSTACK_SECRET_KEY}" ]; then
        ...
        #プラットフォーム番号の取得
        PLATFORM_NO=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -platformname \"${PLATFORM_NAME}\" -platformkind openstack"`
        ...
        SQL_ALREADY_OPENSTACK_CREATED="SELECT USER_NO FROM OPENSTACK_CERTIFICATE where USER_NO =${USER_NO} and PLATFORM_NO=${PLATFORM_NO}"
        USER_NO_CHECK=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -S -sql \"${SQL_ALREADY_OPENSTACK_CREATED}\" -columntype int -columnname USER_NO"`
        ...
        #OPENSTACKユーザー認証情報の作成
        SQL_CREATE_OPENSTACK_CERTIFICATE="INSERT INTO OPENSTACK_CERTIFICATE values (${USER_NO}, '${PLATFORM_NO}','${OPENSTACK_ACCESS_ID}','${OPENSTACK_SECRET_KEY}','${USER_NAME}')"
        MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -U -sql \"${SQL_CREATE_OPENSTACK_CERTIFICATE}\""`
        ...
        #OPENSTACK用のキーペアをAPIを通して送る
        MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -I -keyname \"${USER_NAME}\" -publickey \"${SSH_KEY_PUB}\" -userno \"${USER_NO}\" -platformno \"${PLATFORM_NO}\" -platformkind openstack"`

        if [ -n "${MESSAGE}" ]; then
            SQL_DELETE_OPENSTACK_CERTIFICATE="DELETE FROM OPENSTACK_CERTIFICATE where USER_NO = ${USER_NO} and PLATFORM_NO = ${PLATFORM_NO}"
            if [ "${MESSAGE}" = "IMPORT_SKIPPED" ]; then
                ...
                #エラーの場合はOPENSTACKユーザー認証情報を削除する
                MESSAGE=`su tomcat -c "java ${JAVA_OPTS} -cp ${CLASSPATH} ${MAIN} -U -sql \"${SQL_DELETE_OPENSTACK_CERTIFICATE}\""`
                if [ -n "${MESSAGE}" ]; then
                        echo "${MESSAGE}"
                fi
                exit 1
