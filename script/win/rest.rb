...
class Puppet::Indirector::REST < Puppet::Indirector::Terminus
  ...
  def find(request)
    ...
    begin
      ...
      Puppet.notice "Retry find(request) :%s:%s:%s:%s" % [ detail , request.server , request.port , indirection2uri(request) ]
      retry if cnt < 5
    end
  end
  ...
  def search(request)
    ...
    begin
      ...
      Puppet.notice "Retry search(request) :%s:%s:%s:%s" % [ detail , request.server , request.port , indirection2uri(request) ]
      retry if cnt < 5
    end
  end
